function addContact() {
    ...
    var potential = JSON.parse(localStorage.getItem("potential")) || [];
    potential.push({ name: name, email: email, dni: dni, date: dateStr });
...
window.onload = function () {
    ...
    for (var i = 0; i < potential.length; i++) {
        ...
        dateCell.textContent = potential[i].date;
...
function saveChanges(saveButton) {
    ...
    potential[index] = { name: name, email: email, dni: dni, date: dateStr };
...
function importData() {
    ...
    input.addEventListener("change", function(){
        ...
        reader.onload = function(){
            ...
            localStorage.setItem("potential", JSON.stringify(jsonData));
            location.reload();
        };
