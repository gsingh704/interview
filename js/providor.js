function addContact() {
    ...
    var providor = JSON.parse(localStorage.getItem("providor")) || [];
    providor.push({ name: name, email: email, dni: dni, date: dateStr });
...
window.onload = function () {
    ...
    for (var i = 0; i < providor.length; i++) {
        ...
        dateCell.textContent = providor[i].date;
...
function saveChanges(saveButton) {
    ...
    providor[index] = { name: name, email: email, dni: dni, date: dateStr };
...
function importData() {
    ...
    input.addEventListener("change", function(){
        ...
        reader.onload = function(){
            ...
            localStorage.setItem("providor", JSON.stringify(jsonData));
            location.reload();
        };
